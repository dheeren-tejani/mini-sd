...
def parse_log_line(line):
    ...
    pattern = r'\[.*?\] \[INFO\] Step (\d+) - loss: ([\d.]+) \| recon_loss: ([\d.]+) \| kl_loss: ([\d.]+) \| kl_weight: ([\d.]+) \| lr: ([\d.]+) \| step: (\d+) \| timestamp: ([\d.]+)'
    
    match = re.search(pattern, line)
    if match:
        step, loss, recon_loss, kl_loss, kl_weight, lr, step2, timestamp = match.groups()
...
def plot_metrics(metrics, output_path):
    ...
    axes[1].plot(steps, recon_losses, linewidth=1.5, color='#1f77b4')
    axes[1].set_xlabel('Step', fontsize=11)
    axes[1].set_ylabel('recon_loss', fontsize=11)
    axes[1].set_title('recon_loss over training', fontsize=12)
    axes[1].grid(True, alpha=0.3)
    axes[1].tick_params(labelsize=9)
    
    # Plot 3: KL Loss
    axes[2].plot(steps, kl_losses, linewidth=1.5, color='#1f77b4')
    axes[2].set_xlabel('Step', fontsize=11)
    axes[2].set_ylabel('kl_loss', fontsize=11)
    axes[2].set_title('kl_loss over training', fontsize=12)
...
def main():
    ...
    print("="*80)
    print("VAE TRAINING LOG PARSER & PLOTTER")
    print("="*80)
    
    # Define paths (update these to match your setup)
    log_dir = "./log-dir"  # Current directory, or specify path
    output_json = "vae_training_metrics.json"
    output_plot = "vae_training_curves.png"
    
    # Find all log files matching pattern
    log_pattern = "*vae_training*.log"
